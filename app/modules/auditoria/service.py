...
def sanitize_payload(payload: Any) -> Any:
    """Elimina contraseñas y tokens sensibles de los datos a auditar."""
    if isinstance(payload, dict):
        ...
        # Si es un string JSON válido, parsear, sanitizar y volver a convertir
        try:
...
def registrar_evento(
    ...
    """Registra de forma inmutable un evento en la bitácora de auditoría (RN-CU21-01, RN-CU21-02)."""
    # Si id_clinica no fue pasado, intentar inferirlo del usuario
    if id_clinica is None:
        user = db.query(Usuario).filter(Usuario.id_usuario == id_usuario).first()
        if user and user.id_clinica:
            id_clinica = user.id_clinica
        else:
            id_clinica = 1  # Fallback a clínica principal por defecto
    ...
    # Serializar a string o dejar como JSON compatible
    str_ant = json.dumps(sanitized_ant, ensure_ascii=False) if isinstance(sanitized_ant, (dict, list)) else (str(sanitized_ant) if sanitized_ant is not None else None)
    str_nue = json.dumps(sanitized_nue, ensure_ascii=False) if isinstance(sanitized_nue, (dict, list)) else (str(sanitized_nue) if sanitized_nue is not None else None)
...
def _map_row_to_entry(auditoria: Auditoria, usuario: Optional[Usuario]) -> AuditLogEntry:
    ...
    return AuditLogEntry(
        ...
        tenant_id=str(auditoria.id_clinica),
...
def get_audit_logs(
    ...
    """Consulta paginada con filtros de la bitácora de auditoría."""
...
def export_audit_logs_excel(
    ...
    """Genera un archivo Excel (.xlsx) con los registros filtrados."""
    query = _build_audit_query(
        db=db,
        tenant_id=tenant_id,
        fecha_inicio=fecha_inicio,
    ...
    try:
        ...
        ws = wb.active
        ws.title = "Bitácora de Auditoría"

        # Encabezado del reporte
        ws.merge_cells("A1:H1")
        ws["A1"] = "Hospital San Juan de Dios - Reporte de Bitácora de Auditoría"
        ...
        ws["A2"] = f"Generado el: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')} | Registros: {len(rows)}"
        ...
        # Columnas
        headers = [
            "ID", "Fecha y Hora", "Usuario", "Correo", "Acción",
            "Tabla", "ID Registro", "IP Cliente", "Descripción"
        ]
        ...
        # Fallback a CSV formateado en BytesIO si openpyxl no estuviera disponible
        import csv
        output = io.BytesIO()
        output.write("\ufeff".encode("utf-8")) # BOM UTF-8
...
def export_audit_logs_pdf(
    ...
    """Genera un archivo PDF formal con los registros filtrados."""
    query = _build_audit_query(
        db=db,
        tenant_id=tenant_id,
        fecha_inicio=fecha_inicio,
    ...
    try:
        ...
        # Anchos de columna en landscape letter (aprox 730 pt útiles)
        ...
        # Fallback simple text PDF placeholder si no hay reportlab
